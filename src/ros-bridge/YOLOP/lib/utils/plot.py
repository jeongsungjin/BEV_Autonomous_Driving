...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False):
    ...
    if not is_demo:
        # result(mask) 크기가 입력 이미지와 다를 경우 보간 없이 크기 맞추기
        if result.shape[0] != img.shape[0] or result.shape[1] != img.shape[1]:
            result = cv2.resize(result.astype(np.uint8), (img.shape[1], img.shape[0]), interpolation=cv2.INTER_NEAREST)
