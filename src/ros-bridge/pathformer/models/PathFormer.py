...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Projection: map aggregated features to (pred_len * 2) for XY trajectory
        self.projections = nn.Linear(self.seq_len * self.d_model, self.pre_len * 2)
    ...
    def forward(self, x):
        ...
        # out shape: (batch, seq_len, num_nodes, d_model) -> aggregate over nodes
        out = out.permute(0,2,1,3)  # (batch, num_nodes, seq_len, d_model)
        out = out.mean(dim=1)       # (batch, seq_len, d_model)
        out = out.reshape(batch_size, -1)  # (batch, seq_len*d_model)

        out = self.projections(out)  # (batch, pred_len*2)
        out = out.view(batch_size, self.pre_len, 2)  # (batch, pred_len, 2)
